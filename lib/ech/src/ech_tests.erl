...
basic_doc_test() ->
    ...
    {ok, Db} = ech:open(DbName),

    % Docs are created using ech_doc:new.
    Doc = ech_doc:new(),

    % This is an empty doc, but it's been assigned an ID.
    Id = ech_doc:get_id(Doc),
    ?assert(is_binary(Id)),
    ?assert(size(Id) > 0),

    % It has no revisions.
    ?assertEqual(undefined, ech_doc:get_rev(Doc)),

    % Store the document using put/2. We get back a revised document.
    {ok, DocR} = ech:put(Db, Doc),
    ?assert(ech_doc:get_rev(DocR) =/= undefined),

    % The document current has no attributes.
    ?assertEqual([], ech_doc:get_attr_names(DocR)),

    % We can modify the revised document.
    Doc2 = ech_doc:set_attr(tags, [red, green, blue], DocR),
    {ok, Doc2R} = ech:put(Db, Doc2),
    ?assertEqual([tags], ech_doc:get_attr_names(Doc2R)),
    ?assertEqual([red, green, blue], ech_doc:get_attr(tags, Doc2R)),

    % TODO: finish basic doc ops: get_attrs, del_attr, delete.
    
    ech:delete_db(DbName).
...
batch_insert_test() ->
    ...
    Docs = [ech_doc:new() || _ <- lists:seq(1, 10)],
    DocsR = ech:put_many(Db, Docs),
...
select_docs_test() ->
    ...
    ?assertEqual({0, 0, []}, ech:select(Db, [])),

    % To select from all docs using ID ranges, use ech:select/4. Let's add
    % some docs to select.

    D1 = ech_doc:new("1", [{name, "doc-1"}]),
    D2 = ech_doc:new("2", [{name, "doc-2"}]),
    D3 = ech_doc:new("3", [{name, "doc-3"}]),
    D4 = ech_doc:new("4", [{name, "doc-4"}]),
    ech:put_many(Db, [D1, D2, D3, D4]),

    % We can select every doc in the database ordered by ID as follows:

    {TotalRowCount, Offset, Rows1} = ech:select(Db, []),
    ...
    {4, 0, Rows2} = ech:select(Db, [include_docs]),
    ...
    {4, 2, Rows5} = ech:select(Db, [{skip, 2}]),
...
basic_view_test() ->
    ...
    %
    % ech_nqs supports map functions in the following forms:
    %
    ...
    % this case, we use "ech", which must be registered to use ech_nqs
    % (native query server).
    %
    % To be used as views, the binary strings below are required (as opposed to
    % lists).

    DDoc1 = ech_doc:new("_design/ech_str",
                        [{<<"language">>, <<"ech">>},
                         {<<"views">>, [{<<"by_date">>,
                                         [{<<"map">>, MapStr}]}]}]),
    ech:put(Db, DDoc1),
    ...
    R1 = ech:select({Db, "ech_str", "by_date"}, []),
    ...
    DDoc2 = ech_doc:new("_design/ech_fun",
                        [{<<"language">>, <<"ech">>},
                         {<<"views">>, [{<<"by_date">>,
                                         [{<<"map">>, MapFunBin}]}]}]),
    ech:put(Db, DDoc2),
    ...
    R2 = ech:select({Db, "ech_fun", "by_date"}, []),
    ...
    MF = <<"{ech_tests, map_date_title}.">>,
    DDoc3 = ech_doc:new("_design/ech_mf",
                        [{<<"language">>, <<"ech">>},
                         {<<"views">>, [{<<"by_date">>,
                                         [{<<"map">>, MF}]}]}]),
    ech:put(Db, DDoc3),
    ...
    R3 = ech:select({Db, "ech_mf", "by_date"}, []),
    ...
    MFA = <<"{ech_tests, map_date_title, [myarg]}.">>,
    DDoc4 = ech_doc:new("_design/ech_mfa",
                        [{<<"language">>, <<"ech">>},
                         {<<"views">>, [{<<"by_date">>,
                                         [{<<"map">>, MFA}]}]}]),
    ech:put(Db, DDoc4),
    ...
    R4 = ech:select({Db, "ech_mfa", "by_date"}, []),
...
view_support_test() ->
    ...
    DDoc1 = ech_design:new("ech_fun", [{view, {"by_date", Map}}]),
    ech:put(Db, DDoc1),
    ...
    ech:put(Db, ech_doc:new("biking", [{"title", "Biking"},
                                            {"date", "2009/01/30"}])),
    ech:put(Db, ech_doc:new("hello", [{"title", "Hello World"},
                                           {"date", "2009/01/15"}])),
    ...
    R1 = ech:select({Db, "ech_fun", "by_date"}, []),
    ?assertEqual(Expected, R1),
    ...
    DDoc2 = ech_design:new("ech_mf", [{view, {"by_date", {ech_tests, 
                                                          map_date_title}}}]),
    ech:put(Db, DDoc2),
    R2 = ech:select({Db, "ech_mf", "by_date"}, []),
    ?assertEqual(Expected, R2),
...
term_store_test() ->
    ...
    ech:put(Db, ech_term:new("jane", {user, "jane", admin})),
    ech:put(Db, ech_term:new("adam", {user, "adam", user})),

    % While these documents can't be viewed in Futon, we can read them.

    {ok, Jane} = ech:get(Db, "jane"),
    ?assertEqual({user, "jane", admin}, ech_term:term(Jane)),

    {ok, Adam} = ech:get(Db, "adam"),
    ?assertEqual({user, "adam", user}, ech_term:term(Adam)),
...
composite_id_pattern_test() ->
    ...
    C1 = ech_doc:new("blog-1/post-1/comment-1",
                      [{time, "12:35"},
                       {by, "Mary"},
                       {text, "And bouncy!"}]),
    ech:put(Db, C1),

    C2 = ech_doc:new("blog-1/post-1/comment-2",
                      [{time, "12:45"},
                       {by, "Bob"},
                       {text, "Bouncy?"}]),
    ech:put(Db, C2),
...
composite_key_test() ->
    ...
    ech:put_many(Db, [B, P, C1, C2]),

    % Let's also add some other blogs.

    ech:put(Db, ech_doc:new("102", [{type, blog}])),
    ech:put(Db, ech_doc:new("898", [{type, blog}])),
    ech:put(Db, ech_doc:new("900", [{type, blog}])),

    % Getting all of the documents...

    {7, 0, All} = ech:select(Db, []),
    ...
    % the data using ech_doc/ech_term (which could be imported for
    % compactness of code).
    ...
    ech:put(Db, ech_design:new("blog", [{view, {"list", Map}}])),
    ...
    {_, _, [R1]} = ech:select({Db, "blog", "list"}, ["899"], []),
    ...
    {_, _, R2} = ech:select({Db, "blog", "list"}, ["899"], ["900"],
                                [exclude_end]),
    ...
    {_, _, BlogDocs} = ech:select({Db, "blog", "list"},
                                      ["899"], ["899"++[255]], []),
    ...
    {_, _, PostDocs} = ech:select({Db, "blog", "list"},
                                      ["899", "341"], 
                                      ["899", "341"++[255]], []),
...
fruit_price_test() ->
    ...
    D1 = ech_doc:new([{"item", "apple"},
                      {"prices", [{"Fresh Mart", 1.59},
                                  {"Price Max", 5.99},
                                  {"Apples Express", 0.79}]}]),
    D2 = ech_doc:new([{"item", "orange"},
                      {"prices", [{"Fresh Mart", 1.99},
                                  {"Price Max", 3.19},
                                  {"Citrus Circus", 1.09}]}]),
    D3 = ech_doc:new([{"item", "banana"},
                      {"prices", [{"Fresh Mart", 1.99},
                                  {"Price Max", 0.79},
                                  {"Banana Montana", 4.22}]}]),
    M = fun(Doc) ->
    ...
    DD = ech_design:new("fruit", [{view, {"price", M}}]),
    
    ech:put_many(Db, [D1, D2, D3, DD]),

    {_, _, Rows} = ech:select({Db, "fruit", "price"}, []),
...
web_source_test() ->
    ...
    P1 = ech_doc:new("doc1", [{src, "/foo/bar1"},
                              {body, "title: bar 1\nscore: 10"},
                              {type, "foo"}]),
    P2 = ech_doc:new("doc2", [{src, "/foo/bar2"},
                              {body, "title: bar 2\nscore: 4"}]),
    P3 = ech_doc:new("doc3", [{src, "/baz/bar3"},
                              {body, "title: bar 3"},
                              {type, "baz"}]),
    ech:put_many(Db, [P1, P2, P3]),
    ...
    Indexes = ech_design:new("indexes", [{view, {"type", TypeF}},
                                         {view, {"score", ScoreF}},
                                         {view, {"title", TitleF}}]),
...
basic_map_reduce_test() ->
    ...
    ech:put_many(Db, [ech_doc:new([{key, ["a","b","c"]}]),
                      ech_doc:new([{key, ["a","b","e"]}]),
                      ech_doc:new([{key, ["a","c","m"]}]),
                      ech_doc:new([{key, ["b","a","c"]}]),
                      ech_doc:new([{key, ["b","a","g"]}])]),
    
    M = fun(Doc) -> [{get_value(key, Doc), 1}] end,
    R = fun(_Keys, Vals, _) -> lists:sum(Vals) end,
    ech:put(Db, ech_design:new("test", [{view, {"test", M, R}}])),

    {_, _, Rows} = ech:select({Db, "test", "test"}, []),
